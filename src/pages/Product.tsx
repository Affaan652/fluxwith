...
const features = [
  ...
  },
  {
    ...
    description: 'Every site you build is mobile-first and looks perfect on all devices.',
    icon: (
      <svg className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
        ...
        <div className="mx-auto max-w-6xl">
          ...
          >
            ...
            <div className="mt-6 flex flex-wrap gap-3">
              <Link
                to="/signup"
                className="inline-flex items-center gap-2 rounded-lg bg-white px-5 py-2.5 text-sm font-semibold text-black hover:bg-gray-100 active:scale-[0.96] transition-all duration-150 ease-out"
              >
                Start building free
              ...
              <Link
                to="/showcase"
                className="inline-flex items-center gap-2 rounded-lg border border-white/20 px-5 py-2.5 text-sm font-semibold text-white hover:border-white/40 transition-all duration-150 ease-out active:scale-[0.96]"
              >
                View examples
              </Link>
            </div>
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {features.map((feature, index) => (
              <motion.div
                ...
                className="group rounded-lg border border-white/10 bg-white/5 p-5 hover:border-white/20 transition-colors duration-150"
              >
                <div className="mb-3 inline-flex rounded-lg bg-white/10 p-2.5 text-white transition-colors group-hover:bg-white group-hover:text-black">
                  {feature.icon}
                </div>
                <h3 className="text-base font-semibold text-white">{feature.title}</h3>
                <p className="mt-2 text-xs text-gray-400 leading-relaxed">{feature.description}</p>
              </motion.div>
            ))}
          </div>
        </div>
      ...
      <section className="bg-white py-12 px-6">
        <div className="mx-auto max-w-xl text-center">
          <motion.div
            ...
            transition={{ duration: 0.3, ease: [0.16, 1, 0.3, 1] }}
          >
            <h2 className="text-2xl font-bold tracking-tight text-black">Ready to get started?</h2>
